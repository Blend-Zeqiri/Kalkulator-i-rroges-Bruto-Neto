...
export default function Calculator() {
  ...
  <main className="min-h-screen bg-[#f2f7fc] px-4 py-6 text-[#16243a] transition-colors dark:bg-[#0b1220] dark:text-slate-100 sm:px-6 sm:py-10">
    <div className="mx-auto w-full max-w-4xl">
      <header className="mb-10 flex items-start justify-between gap-6 border-b border-[#d7e2ee] pb-6 dark:border-slate-800">
        <div><p className="mb-2 text-xs font-semibold uppercase tracking-[.16em] text-[#3478c8] dark:text-blue-300">{text.country} · {currency.symbol}</p><h1 className="text-2xl font-semibold tracking-[-.035em] sm:text-3xl">{text.title}</h1><p className="mt-2 text-sm text-[#60728a] dark:text-slate-300">{text.subtitle}</p></div>
        <button type="button" onClick={toggleTheme} aria-label={darkMode ? text.lightMode : text.darkMode} title={darkMode ? text.lightMode : text.darkMode} className="grid size-10 shrink-0 place-items-center rounded-full border border-[#8ab8e8] bg-white text-lg text-[#3478c8] transition hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3478c8] dark:border-slate-700 dark:bg-slate-900 dark:text-blue-300 dark:hover:bg-slate-800">
      ...
      <section className="overflow-hidden rounded-2xl border border-[#d7e2ee] bg-white shadow-[0_12px_32px_rgba(40,83,130,.07)] dark:border-slate-800 dark:bg-slate-950 dark:shadow-[0_12px_32px_rgba(0,0,0,.2)]">
        <div className="grid border-b border-[#e2ebf4] dark:border-slate-800 md:grid-cols-[1fr_auto_1fr]">
          <div className="p-5 sm:p-7">
            <label htmlFor="paga" className="block text-xs font-semibold uppercase tracking-[.14em] text-[#60728a] dark:text-slate-300">{inputLabel}</label>
            <div className="mt-3 flex items-baseline border-b border-[#bfd2e6] pb-2 focus-within:border-[#3478c8] dark:border-slate-700 dark:focus-within:border-blue-400">
              <span className="text-2xl text-[#60728a] dark:text-slate-300">{currency.symbol}</span>
              <input id="paga" type="number" inputMode="decimal" min={input.minimum} step={input.step} value={amount} onChange={event => setAmount(event.target.value)} placeholder={input.placeholder} className="min-w-0 flex-1 bg-transparent pl-2 text-4xl font-medium tracking-[-.04em] outline-none placeholder:text-[#d9e4ef] dark:text-white dark:placeholder:text-slate-700" />
            </div>
          </div>

          <div className="hidden w-px bg-[#e2ebf4] dark:bg-slate-800 md:block" />

          <div className="bg-[#4f8fd4] p-5 text-white dark:bg-[#183b66] sm:p-7">
        ...
        <div className="grid md:grid-cols-[260px_1fr]">
          <aside className="border-b border-[#e2ebf4] bg-[#f7faff] p-5 dark:border-slate-800 dark:bg-[#101827] sm:p-7 md:border-b-0 md:border-r" aria-label={text.calculationType}>
            ...
            <div className="space-y-2" role="radiogroup" aria-label={text.calculationType}>
              {modes.map(({ value, label }) => <button type="button" role="radio" aria-checked={mode === value} key={value} onClick={() => setMode(value)} className={`flex w-full items-center justify-between rounded-lg border px-3.5 py-3 text-left text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3478c8] dark:focus-visible:ring-blue-400 ${mode === value ? 'border-[#8ab8e8] bg-white font-semibold text-[#3478c8] shadow-sm dark:border-blue-500 dark:bg-slate-900 dark:text-blue-300' : 'border-transparent text-[#60728a] hover:bg-white dark:text-slate-400 dark:hover:bg-slate-900'}`}><span>{label}</span><span className={`size-2 rounded-full ${mode === value ? 'bg-[#4f8fd4] dark:bg-blue-400' : 'bg-[#cfdeec] dark:bg-slate-700'}`} /></button>)}
            </div>
          ...
          <div className="p-5 sm:p-7">
            ...
            <div className="divide-y divide-[#e2ebf4] border-y border-[#d7e2ee] dark:divide-slate-800 dark:border-slate-800">
            ...
            <div className="mt-7">
              ...
              <div className="row mt-2 border-t border-[#d7e2ee] dark:border-slate-800"><span className="row-label font-semibold text-[#16243a] dark:text-slate-200">{text.totalTax}</span><span className="row-value text-[#9f3e3e] dark:text-rose-400">−{formatMoney(result.tax)}</span></div>
            </div>
          </div>
        </div>
      </section>
