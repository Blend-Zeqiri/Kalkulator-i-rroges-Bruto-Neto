...
export default function Calculator() {
  ...
  return <main className="min-h-screen px-4 py-6 sm:px-6 sm:py-10">
    <div className="mx-auto w-full max-w-4xl">
      <header className="mb-10 flex items-start justify-between gap-6 border-b border-[#d9d6cf] pb-6">
        <div><p className="mb-2 text-xs font-semibold uppercase tracking-[.16em] text-[#1f3a5f]">{text.country} · {currency.symbol}</p><h1 className="text-2xl font-semibold tracking-[-.035em] sm:text-3xl">{text.title}</h1><p className="mt-2 text-sm text-[#697080]">{text.subtitle}</p></div>
        <div className="hidden size-10 place-items-center rounded-full border border-[#1f3a5f] text-sm font-semibold text-[#1f3a5f] sm:grid" aria-hidden>{currency.symbol}</div>
      </header>

      <section className="overflow-hidden rounded-2xl border border-[#d9d6cf] bg-[#fffefa] shadow-[0_12px_32px_rgba(23,32,51,.045)]">
        <div className="grid border-b border-[#dedbd4] md:grid-cols-[1fr_auto_1fr]">
          <div className="p-5 sm:p-7">
            <label htmlFor="paga" className="block text-xs font-semibold uppercase tracking-[.14em] text-[#697080]">{inputLabel}</label>
            <div className="mt-3 flex items-baseline border-b border-[#bfc2c9] pb-2 focus-within:border-[#1f3a5f]">
              <span className="text-2xl text-[#697080]">{currency.symbol}</span>
              <input id="paga" type="number" inputMode="decimal" min={input.minimum} step={input.step} value={amount} onChange={event => setAmount(event.target.value)} placeholder={input.placeholder} className="min-w-0 flex-1 bg-transparent pl-2 text-4xl font-medium tracking-[-.04em] outline-none placeholder:text-[#d7d5d0]" />
            </div>
          </div>

          <div className="hidden w-px bg-[#dedbd4] md:block" />

          <div className="bg-[#1f3a5f] p-5 text-white sm:p-7">
        ...
        <div className="grid md:grid-cols-[260px_1fr]">
          <aside className="border-b border-[#dedbd4] bg-[#faf9f6] p-5 sm:p-7 md:border-b-0 md:border-r" aria-label={text.calculationType}>
            <p className="mb-3 text-xs font-semibold uppercase tracking-[.14em] text-[#8b909b]">{text.calculationType}</p>
            <div className="space-y-2" role="radiogroup" aria-label={text.calculationType}>
              {modes.map(({ value, label }) => <button type="button" role="radio" aria-checked={mode === value} key={value} onClick={() => setMode(value)} className={`flex w-full items-center justify-between rounded-lg border px-3.5 py-3 text-left text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1f3a5f] ${mode === value ? 'border-[#1f3a5f] bg-white font-semibold text-[#1f3a5f]' : 'border-transparent text-[#697080] hover:bg-white'}`}><span>{label}</span><span className={`size-2 rounded-full ${mode === value ? 'bg-[#1f3a5f]' : 'bg-[#d5d3cd]'}`} /></button>)}
            </div>

            <p className="mb-3 mt-8 text-xs font-semibold uppercase tracking-[.14em] text-[#8b909b]">{text.contributions}</p>
          ...
          <div className="p-5 sm:p-7">
            <div className="mb-4 flex items-end justify-between gap-4"><div><h2 className="font-semibold">{text.summary}</h2><p className="mt-1 text-xs text-[#8b909b]">{text.taxRates}: {taxBrackets.map(({ rate }) => `${rate}${text.percentSymbol}`).join(text.rateSeparator)}</p></div><span className="text-xs font-medium text-[#697080]">{modes.find(item => item.value === mode)?.label}</span></div>

            <div className="divide-y divide-[#ebe9e4] border-y border-[#dedbd4]">
            ...
            <div className="mt-7">
              <p className="mb-1 text-xs font-semibold uppercase tracking-[.14em] text-[#8b909b]">{text.incomeTax}</p>
              ...
              <div className="row mt-2 border-t border-[#dedbd4]"><span className="row-label font-semibold text-[#172033]">{text.totalTax}</span><span className="row-value text-[#9f3e3e]">−{formatMoney(result.tax)}</span></div>
            </div>
          </div>
        </div>
      </section>
      <footer className="mt-5 text-center text-xs text-[#9297a1]">{text.subtitle}</footer>
    </div>
  </main>;
}

function CompactContribution({ label, rate, setRate }: Pick<ContributionRowProps, "label" | "rate" | "setRate">) {
  return <div className="mb-3"><p className="mb-2 text-xs text-[#697080]">{label}</p><Stepper value={rate} onChange={setRate} label={label.toLowerCase()} minimum={contributions.minimum} maximum={contributions.maximum} /></div>;
}
...
function TaxRow({ label, rate, value, deduction = false }: TaxRowProps) {
  return <div className="row py-2.5"><span className="text-sm text-[#697080]">{label}{rate && <span className="ml-2 rounded border border-[#d9d6cf] px-1.5 py-0.5 text-[10px] font-semibold text-[#697080]">{rate}</span>}</span><span className={`row-value ${deduction ? 'text-[#9f3e3e]' : 'text-[#a3a6ad]'}`}>{formatMoney(value)}</span></div>;
}
